<script lang="ts">
  ...
  import DiffViewer from "./commit/DiffViewer.svelte";
  ...
  async function loadDiff(file: FileStatus) {
      ...
      diffContent = "";
      try {
          diffContent = await GitService.getDiff(file.path, file.staged, repoPath);
      } catch (e: any) {
...
<div class="flex h-full w-full bg-[#0d1117] overflow-hidden text-[#c9d1d9]">
    ...
    <div class="flex-1 flex flex-col bg-[#0d1117] overflow-hidden">
        ...
        {:else}
             ...
             <DiffViewer diff={diffContent} loading={loadingDiff} />
        {/if}
    </div>
</div>
